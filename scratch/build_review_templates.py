...
# Dictionary of airport specific details
airports = {
    "kuaili-review.html": {
        "name": "快狸机场",
    ...
    "huanyuyun-review.html": {
        ...
        "coupon_desc": "全场 9 折",
        "advice": "多设备家庭共享首选。",
        "highlights": ["<strong>全平台支持：</strong> 覆盖 iOS, Android, Windows, macOS。"]
    }
}

# Template HTML generation function
def generate_review_html(filename, info):
    plans_rows = ""
    for p in info["plans"]:
        plans_rows += f'''        <tr>
          ...
          <td>{p["yearly"]}</td>
          <td>{p["limit"]}</td>
          <td><a href="{info["aff_url"]}" target="_blank" class="btn-table-buy">查看 {p["name"].split()[0]} 价格</a></td>
        </tr>\n'''
...
{plans_rows}                            </tbody>
  ...
  <script src="../script.js"></script>
    ...
    with open(os.path.join(articles_dir, filename), "w", encoding="utf-8") as f:
        f.write(html)
    print(f"Generated review template for {filename}")

for fname, info in airports.items():
    generate_review_html(fname, info)

print("ALL_REVIEW_TEMPLATES_GENERATED")
