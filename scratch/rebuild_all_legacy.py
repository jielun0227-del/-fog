...
def clean_legacy_file(filepath):
    with open(filepath, 'r', encoding='utf-8', errors='ignore') as f:
        html = f.read()

    if '<header class="header">' not in html and '椰汁' not in html and '鏈哄満' not in html:
        return

    # Extract title
    title_match = re.search(r'<title>(.*?)</title>', html, re.DOTALL)
    title = title_match.group(1).split(' - ')[0] + " - 机场Fog 测评博客" if title_match else "网络与代理科普 - 机场Fog 测评博客"
    ...
    if not body_match:
        body_match = re.search(r'<main class="main-content">(.*?)</main>', html, re.DOTALL)
    body = body_match.group(1) if body_match else ""

    # Extract H1
    h1_match = re.search(r'<h1>(.*?)</h1>', html, re.DOTALL)
    h1 = h1_match.group(1) if h1_match else "网络与代理科普"
  ...
  <main class="main-content">
    ...
    <section class="article-hero">
      ...
      <div class="article-meta">
        ...
        <span class="meta-item">独立客观点评 • 科学上网指南</span>
      </div>
    </section>
...
{FOOTER}"""
    ...
    with open(filepath, 'w', encoding='utf-8') as f:
        f.write(new_html)
    print(f"Rebuilt clean: {filepath}")
