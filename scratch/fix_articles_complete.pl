...
foreach my $filepath (@files) {
    ...
    # 1. Legacy site name cleanup
    $content =~ s{椰汁网络日志}{机场Fog 测评博客}g;
    $content =~ s{椰汁评测组}{机场Fog 测评团队}g;
    $content =~ s{椰汁网络}{机场Fog 测评博客}g;
    $content =~ s{椰汁}{机场Fog}g;

    # 2. Convert unparsed markdown bold **text** to HTML <strong>text</strong>
    $content =~ s{\*\*([^\*\n]+)\*\*}{<strong>$1</strong>}g;

    # 3. CSS stylesheet link fix
    ...
    # Remove any extra nested or stray headers
...
print "ALL_ARTICLES_SUCCESSFULLY_FORMATTED\n";
