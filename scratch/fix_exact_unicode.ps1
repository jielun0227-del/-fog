...
$nav2 = "$([char]0x77E5)$([char]0x8666)$([char]0x5E93)" # 知识库
...
foreach ($file in $htmlFiles) {
    ...
    # Clean legacy title and text names
    $content = $content -replace "椰汁网络日志", $siteTitle
    $content = $content -replace "椰汁评测组", $teamTitle
    $content = $content -replace "椰汁网络", $siteTitle
    $content = $content -replace "椰汁", $brandName
...
Write-Output "EXACT_UNICODE_FINISHED"
